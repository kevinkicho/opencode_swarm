...
| 7.Q34 | opencode-frozen recurrence on orchestrator-worker | **DIAGNOSED — REAL CAUSE IS GEMMA PSEUDO-TOOL-CALLS** 2026-04-26 — Q37 fixed the silent-drop but Q34 had a deeper cause. Q34-verify run (run_mog0axza_pzalr8): intro post empirically landed (1343-char user msg + 518-char orchestrator response, vs zero before Q37). But the planner sweep response was a `<think>...</think>` block followed by **literal text containing pseudo-tool-call markup**: `<\|tool>glob{pattern:"..."}<tool\|>`, `<\|tool>read{filePath:"..."}<tool\|>`, etc. — NONE of which are real opencode tool calls (`parts.tools` is empty). GEMMA is roleplaying tool-call syntax as plain text instead of actually calling the tools. Result: planner sweep returned 0 todos, auto-ticker never started, run was dead-on-arrival. Same pathology likely caused both prior `opencode-frozen` events (model loops emitting pseudo-tool-text, F1 watchdog stops it after 10min). Fix paths: (A) per-pattern model swap — orchestrator-worker.teamModels[0] back to a model with reliable tool-use (NEMOTRON tested as orchestrator hit step-loop cost issue earlier; could try opencode-zen/sonnet for reliability if user budgets it); (B) prompt engineering — modify orchestrator intro + planner sweep prompts to explicitly forbid narrating tool calls and require actual `tool` part emission; (C) accept orchestrator-worker as currently broken under GEMMA monoculture, document the limitation, fix later. Recommend (B) — try one prompt revision before swapping models. | A subsequent orchestrator-worker run produces a planner-sweep response with `parts.tools` non-empty (real todowrite call) instead of pseudo-tool-call text. If still broken after prompt fix, escalate to (A). |
