...
| 7.Q42 | Workers (GEMMA) produce pseudo-tool-text in orch-worker context | **PARTIALLY SHIPPED** 2026-04-26 — fix path (D) shipped. `lib/server/blackboard/coordinator/dispatch.ts` now applies a phantom-completion guard between turn-completion and done-transition: if `editedPaths.length === 0` AND the worker produced ZERO real `tool` parts AND ZERO real `patch` parts AND the text doesn't begin with `skip:`, bounce to stale via `retryOrStale` with note `[phantom-no-tools]` instead of marking done. Legitimate cases still pass: research-only work has real tool parts; explicit "skip: <reason>" replies match the regex; actual file edits have patch parts. Phantom commits cannot ship under any model behavior. Open hypotheses (A) session-creation order, (B) opencode pool state, (C) ollama-cloud drift not yet investigated — but the guard makes them irrelevant for correctness; the worst case now is "todo bounces back to stale" instead of "phantom commit shipped". | tsc clean, 254/254 tests green. Empirical verifier: spawn an orchestrator-worker run on a directive that triggers the worker pseudo-tool-text behavior; confirm todos transition to `stale` with note `[phantom-no-tools]` instead of `done`. Re-run blackboard separately for (C) probe (deferrable). |
