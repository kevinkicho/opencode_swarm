...
| 7.Q26 | Decomposition wave 2 (next 8 large files) | **IN PROGRESS** 2026-04-26 — pass 1 of N: extracted `app/page-internals/use-modal-state.ts` (custom hook for the 8 modal flag/setter pairs with stable openers/closers) + `app/page-internals/page-modals.tsx` (component owning the 8 dynamic-imported overlay renders). `app/page.tsx` 1392 → 1321 lines (-71). Remaining ≥500-line files unchanged: `new-run-modal.tsx` (900), `swarm-timeline.tsx` (779), `retro-view.tsx` (629), `spawn-agent-modal.tsx` (626), `board-rail.tsx` (623), `turn-cards-view.tsx` (619), `agent-roster.tsx` (557), `contracts-rail.tsx` (513). Future passes will extract page-side hooks (selection state, cost-cap watcher, palette wiring) then move to component-side splits. | tsc clean, 254/254 tests green, Playwright smoke-test confirms page renders + inspector populates after extraction. Pass 2+ will continue chipping at app/page.tsx. |
