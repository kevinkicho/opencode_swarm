...
| 6.7 | **Auto-stick-to-bottom across ALL panels** | 2h | **PARTIAL** — timeline fixed via state-machine (`504c342`, Playwright-verified). Other panels (board-rail, plan-rail, contracts, iterations, debate, roles, map, council, phases, strategy, heat) still render top-anchored. Real remaining work: lift the at-bottom-state pattern into a shared hook + apply to each rail's scroll container. |
| 6.8 | **`latest ↓` button visibility audit** | 1h | **TIMELINE PASS** (`504c342`, audit-verified at threshold 80px). Other panels (the same list as 6.7) still don't have a scroll-to-latest button. Pairs with 6.7. |
