...
| 7.Q26 | Decomposition wave 2 (next 8 large files) | **IN PROGRESS** 2026-04-26 — `app/page.tsx` 1392 → 1208 lines (-184, 13.2%) across 4 passes. Pass 1: `use-modal-state.ts` (8 modal flag pairs → stable openers/closers) + `page-modals.tsx` (8 dynamic overlay renders). Pass 2: `use-selection-state.ts` (focusedMsgId / selectedAgentId / selectedFileHeat / drawerOpen + 7 handlers enforcing "set one, clear others" invariant; selectSession bridges sessionID→agentID via agents array). Pass 3: `use-cost-cap-block.ts` (costCapBlock state + safePost wrapper that turns CostCapError into the banner side-effect + dismissCap + auto-clear-on-swarmRunID-change effect). Pass 4: `use-global-keybindings.ts` (Cmd-K palette toggle / Cmd-N new run; consolidates keyboard handler effect). Remaining ≥500-line files unchanged: `new-run-modal.tsx` (900), `swarm-timeline.tsx` (779), `retro-view.tsx` (629), `spawn-agent-modal.tsx` (626), `board-rail.tsx` (623), `turn-cards-view.tsx` (619), `agent-roster.tsx` (557), `contracts-rail.tsx` (513). PageBody current breakdown: ~340 lines hooks/state, ~398 lines JSX render. | tsc clean, 254/254 tests green, Playwright smoke-test confirms page renders + inspector populates after each pass. Future passes: JSX render-block splits (pattern-rail switch, SwarmComposer wiring), then component-side files. |
...
| 7.Q34 | opencode-frozen recurrence on orchestrator-worker | DIAGNOSIS — needs investigation. 2/2 orchestrator-worker validation runs hit `opencode-frozen` ~12 minutes in (run_mofh1sua_9yl4s9 session 1, run_mofy1jz1_6b3ofx session 2). Same pattern, same minute mark, same model (GEMMA orchestrator), F1 watchdog correctly catches both. Root cause unclear: could be (A) GEMMA-specific failure on long-context orchestrator role (todowrite step-loop?), (B) opencode-side hang on a specific tool sequence, (C) a race in our dispatch logic specific to the orchestrator-excluded worker fan-out. Mitigation: F1 watchdog stops the run cleanly and rollup auto-fires (Q20+Q24) so partial output is preserved. Suggested investigation: capture opencode debug log during the freeze window; run with NEMOTRON in the orchestrator seat to see if the failure is GEMMA-specific. | A subsequent orchestrator-worker run completes without hitting opencode-frozen (or the root cause is identified + mitigated). |
| 7.Q35 | Picker can't distinguish opencode-frozen from clean stop | OPEN — Q27 fix correctly classified MessageAbortedError sessions as `idle`, but that means a cap-stopped run (success) and an opencode-frozen run (silent failure) BOTH show as `idle` with the same dot color. User looking at the picker can't tell which runs completed cleanly vs which silently died. Fix: when classifying run status, read the ticker snapshot's `stopReason` field. If `opencode-frozen` / `zen-rate-limit` / `replan-loop-exhausted` (failure modes), classify as `error` regardless of session-tail shape. If `wall-clock-cap` / `commits-cap` / `todos-cap` / `manual` / `auto-idle` (graceful terminations), keep `idle`. Need to thread ticker.stopReason through deriveRunRow alongside the existing ticker-active check. | A run with stopReason=`opencode-frozen` shows as `error` in the picker; a run with stopReason=`commits-cap` shows as `idle`. |
| 7.Q36 | Validation driver wastes wallclock when ticker stops early | OPEN — `scripts/_validation-8-patterns.sh active_sleep`. Observed validation-8 session 2: run 2 hit `opencode-frozen` at minute 12, but the driver kept sleeping until minute 60 (~48 min wasted) before firing /stop and advancing to run 3. Fix: inside the active_sleep poll loop (already polling every PROBE_INTERVAL), additionally probe `/board/ticker` and break early when `stopped: true`. For non-ticker patterns the kickoff coroutine signals via `finalizeRun` — could add an analogous "is run finalized" probe (rollup row count > 0?). Script-side enhancement, not a product bug; mostly affects validation-run wallclock not user-facing flow. | A pattern that hits opencode-frozen / cap / etc. early advances to the next pattern within PROBE_INTERVAL (5 min) instead of waiting out the full 60-min window. |
| 7.Q37 | No regression test for invalid `agent` param silent-drop | OPEN — `reference_opencode_agent_silent_drop.md` documents the trap (opencode 204s and drops POSTs whose agent param isn't a built-in). Q33 reintroduced this bug despite the memory entry. Defense: (A) lint rule via grep in CI that rejects any `agent: 'X'` literal where X isn't in the built-in set; (B) runtime guard in `postSessionMessageBrowser` / `postSessionMessageServer` that throws on invalid agent values; (C) typed agent enum that makes invalid strings a TS error. Recommend (C) — caught at compile time, no runtime cost, follows existing typed-vocabulary pattern (see opencode-vocabulary.md). | tsc fails when code passes a non-built-in string to the agent field. |
| 7.Q38 | Inspector on aborted-but-rendered sessions | OPEN — edge case not yet repro'd live, but worth probing: once `stopAutoTicker` or `finalizeRun` aborts a session, opening the inspector for that session's last message MIGHT show stale data (e.g., a turn frozen mid-stream with `parts=[]` or pre-abort tokens that were buffered client-side). Inspector reads from `messages` array which is transformed from `liveData.messages` — if the SSE stream stayed open past the abort and emitted an out-of-order partial, the inspector could render an inconsistent slice. Probe: spawn a run, abort mid-turn, click that turn's chip in the timeline immediately + 30s later — verify inspector shows the same final-state content both times. If divergence: lock the inspector's view to a snapshot at click-time rather than reading mutating live data. | Empirical Playwright probe — abort a run mid-turn, click the trailing chip at T0 and T+30s, assert identical inspector body text. |
| 7.Q39 | Runs-list cache TTL too loose for "I just spawned" feedback | OPEN — `useSwarmRuns` polls `/api/swarm/run` every 4s by default. Combined with the meta cache TTL (`META_CACHE_TTL_MS = 2000`), worst-case time-to-visibility for a new run is ~6s. User expectation: instant after spawn. Suggested fix: on `POST /api/swarm/run` success, the new-run flow should `queryClient.invalidateQueries(SWARM_RUNS_QUERY_KEY)` before navigating to the new run's URL — that forces a refetch which bypasses the 4s polling cycle. Alternatively, lower the polling interval when the picker is open (already `intervalMs: 4000` from `useSwarmRuns(4000)` — could be 1500 when picker visible). | Timing probe: spawn a run via `POST /api/swarm/run`, immediately open the picker — new run appears within ~500ms instead of up to 6s. |
| 7.Q40 | /retro page UX for failure-mode runs | OPEN — Q20+Q24 auto-fire rollups for every run-end including failures (opencode-frozen, replan-loop-exhausted, etc.). Verified live on validation-8 session 2 run-2 (opencode-frozen, agent×3+retro×1 rollup landed). But the retro view's outcome banner currently uses a single dot color tone (`OUTCOME_TONE` map at retro-view.tsx:71) and doesn't prominently surface "this run failed at minute X with reason Y" — the user has to read the retro body to find it. Fix: add a header-level failure chip when retro.outcome is `aborted`/`failed`/`partial`, and pull `stopReason` + `stoppedAtMs` from the ticker snapshot (or from a new field on RunRetro) so the chip can read "stopped at min 12 · opencode-frozen". Pairs naturally with 7.Q35 (picker visual distinction) — same data source. | A retro page for a stopReason=opencode-frozen run shows a prominent failure header with the exact minute mark + reason, not just a tonal cue. |
