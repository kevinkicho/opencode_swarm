...
| 7.Q34 | opencode-frozen recurrence on orchestrator-worker | **SHIPPED** 2026-04-26 — `lib/swarm-patterns.ts` orchestrator-worker.teamModels swapped from `Array(n).fill(GEMMA)` to `[GLM, ...rest GEMMA]`. Three-fix sequence: Q33 caught the initial silent-drop on `agent: 'orchestrator'`; Q37's typed-enum upgrade caught the same pattern in the kickoff intro post (verified live: intro now lands as a 1343-char user msg with a 518-char response, vs zero pre-fix); Q34-verify run (run_mog0axza_pzalr8) surfaced the deeper cause — GEMMA on the long-context planner sweep prompt (~37K chars: README + board) emits **pseudo-tool-call text** instead of calling todowrite (`<\|tool>glob{...}<tool\|>` as plain text, parts.tools=[]). Same pathology almost certainly caused both prior `opencode-frozen` events (model spinning on pseudo-tool-text until F1 watchdog catches it). Per the "all non-planner seats → GEMMA" directive, the orchestrator IS the planner-equivalent (it runs `runPlannerSweep`); GLM was already proven on blackboard's identical planner prompt. Workers stay on GEMMA — they don't run the planner sweep. | A subsequent orchestrator-worker run produces a planner-sweep response with `parts.tools` non-empty (real todowrite call), seeds N≥6 todos, auto-ticker starts, no opencode-frozen at min 12. |
