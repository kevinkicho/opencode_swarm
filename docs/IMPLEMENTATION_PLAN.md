...
| 7.Q42 | Workers (GEMMA) produce pseudo-tool-text in orch-worker context | **PARTIALLY SHIPPED** 2026-04-26 — fix path (D) shipped. `lib/server/blackboard/coordinator/dispatch.ts` now applies a phantom-completion guard between turn-completion and done-transition: if `editedPaths.length === 0` AND the worker produced ZERO real `tool` parts AND ZERO real `patch` parts AND the text doesn't begin with `skip:`, bounce to stale via `retryOrStale` with note `[phantom-no-tools]` instead of marking done. Legitimate cases still pass: research-only work has real tool parts; explicit "skip: <reason>" replies match the regex; actual file edits have patch parts. Phantom commits cannot ship under any model behavior. Hypothesis (A) [session-creation asymmetry] and (B) [opencode pool state] **weakened by code-level diff** 2026-04-26 — both patterns leave workers idle until the auto-ticker fires the first work prompt, both use identical `buildWorkPrompt`, both create sessions with the same default `build` agent, and the `excludeSessionIDs` flag only changes which sessions get DISPATCHED, not how they're created. No spawn-level mechanism for differential worker priming. Hypothesis (C) [ollama-cloud GEMMA drift] is now the leading candidate — testable by spawning a blackboard run NOW and checking whether its GEMMA workers also produce pseudo-tool-text. If they do, (C) confirmed; if they don't, the difference is something subtler (different work-prompt content per pattern's directive interpretation? per-session HTTP context state?). | tsc clean, 254/254 tests green. (C) empirical probe: re-run blackboard, count real tool calls per worker session — if zero → (C) confirmed → mitigations are (1) the phantom-completion guard already shipped + (2) consider switching workhorse model away from GEMMA for a stable period. |
