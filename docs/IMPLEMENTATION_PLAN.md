...
| 7.Q42 | Workers (GEMMA) produce pseudo-tool-text in orch-worker context | OPEN — surfaced by Q34-verify-v2 (run_mog101p0_7js6lt) on 2026-04-26. Both worker sessions received 4 work prompts each, responded with 932/637/2018/3826-char text bodies containing `<tool>glob<arg_key>...</arg_value></tool\|>` style pseudo-XML, **zero real opencode tool calls, zero patches, zero workspace file modifications** during the entire run. Coordinator's done-transition is fooled by non-empty assistant text → 9 phantom done-todos. CONFOUND: blackboard's GEMMA workers in validation-8 session 2 (run_mofvukce_glyevj, today, same model + same opencode + same workspace + structurally-identical work prompts) called 47 real tools per worker session. So GEMMA-on-workers IS reliable in some contexts, broken in this one. Hypotheses to investigate: (A) session-creation order or timing differs between blackboard and orchestrator-worker patterns; (B) opencode's session pool has shared state that primes GEMMA differently; (C) ollama-cloud GEMMA response distribution drifted intra-day (would also affect blackboard if re-tested now); (D) the coordinator's "done" transition logic itself has a parser bug that accepts pseudo-tool-XML as evidence of work. (D) is independently fixable — the coordinator should check `info.parts` for actual `tool`/`patch` parts before transitioning, not just check for non-empty assistant text. Recommend tackling (D) first since it's a fix regardless of which model behavior is "correct": phantom commits should never be allowed. Need to read `lib/server/blackboard/coordinator/dispatch.ts` done-transition logic to understand the current criteria. | (D) fix: a worker turn that produces 0 real tool/patch parts gets rejected as "no work", todo bounces back to open or to stale, not transitioned to done. Plus a probe — re-run blackboard now to test (C). |
