...
| 7.Q34 | opencode-frozen recurrence on orchestrator-worker | **VERY LIKELY RESOLVED BY Q37** 2026-04-26 — pending verification. Q37's typed-enum upgrade compile-time-caught a silent-drop bug in `lib/server/orchestrator-worker.ts:113`: every orchestrator-worker run's intro post had `agent: 'orchestrator'` (not a built-in), so opencode silently 204'd it. The orchestrator session has been running without its role-framed intro across every orch-worker run we've ever launched. Without role framing, GEMMA's behavior on the planner sweep may have looped on todowrite (the failure shape we kept observing). Fix landed in Q37 commit; orch-worker now omits the agent param so the intro actually lands. | A future orchestrator-worker run completes without hitting `opencode-frozen` (or hits it for a different reason that surfaces via dev log). If the freeze recurs at ~min 12 with the intro now landing, that rules out the missing-intro hypothesis and shifts back to GEMMA-specific or opencode-internal as the prime suspect. |
