...
| 7.Q19 | **MISPLACEMENT — pattern-specific tabs are in the LEFT panel; should be in the MAIN viewport.** I shipped all 9 per-pattern observability views (`contracts`, `iterations`, `debate`, `roles`, `map`, `council`, `phases`, `strategy`, plus stigmergy heat-decoration) as left-panel tabs in `components/left-tabs.tsx`. The left panel's role is navigation + secondary info (plan, roster, board summary, heat). The main viewport is where the user actually FOCUSES — currently has `timeline / cards / board` toggles. Pattern-specific deep views (debate matrix, strategy revision timeline, council convergence per round, etc.) absolutely belong in the main viewport — they ARE the primary surface for understanding the run on that pattern.<br><br>**Scope:** move 8 components from `LeftTabs` → main-view runView toggle. Per-pattern visibility flags (`showCouncilTab`, `showPhasesTab`, etc.) move with them. Left panel keeps just `plan / roster / board / heat` (the cross-pattern surfaces). Each pattern adds ONE new runView option that's visible only when `boardPattern === <pattern>`.<br><br>**Files affected:**<br>• `components/left-tabs.tsx` — drop 8 tab cases<br>• `app/page.tsx` — widen `runView` union, add render branches<br>• `components/{contracts,iterations,debate,roles,map,council,phases,strategy}-rail.tsx` — adjust embedded mode for the wider main-view container width<br>• Drop the LeftTabs `Tab` union members for these 8<br><br>**Queued (user 2026-04-24): "make swarm-modes specific views appear on mainview, not left panel."** | OPEN | ~6h refactor; needs Playwright probe per pattern after move |
