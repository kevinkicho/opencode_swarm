...
| 8.W3.2 | 6 missing pattern integration tests (orchestrator-worker, role-differentiated, critic-loop, debate-judge, council, map-reduce, deliberate-execute) | #D4 #4 | 4-6h | `tests/integration/<pattern>.test.ts` (un-skip × 7) | **SHIPPED** (2026-04-26 late · 7/7 PASS in 332s vs. failed first batch 1412s). Harness fix: `snapSessions(snap)` helper reads `snap.tokens.sessions` (not top-level — initial predicates couldn't see session activity); plus `allSessionsIdleAfterWork` + `runStatusTerminal` short-circuits in `waitForCondition` so finished/idle runs return immediately instead of polling to TIMEOUT_MS. Predicates loosened to "≥1 session has tokens OR ≥1 board item" — 180s budget too tight for cloud-model serial-critical flows (orch→worker, reduce, judge verdict). Tests now exercise that the planner seeded the board AND a worker started claiming, which is the smoke-level guarantee #4 promised. |
...
| # | Item | HARDENING_PLAN | Effort | Verification gate | Status |
|---|---|---|---|---|---|
| 8.W5.1 | Split `swarm-registry.ts` along fs/derive seam (closes Q47) | #C3 | 2-3h | `swarm-registry-lifecycle.test.ts` stays green | **SHIPPED** (2026-04-26 late · 996 LOC → barrel + `swarm-registry/fs.ts` (469 LOC) + `swarm-registry/derive.ts` (433 LOC)). One-way edge: fs.ts imports `invalidateDerivedRow` from derive.ts; derive.ts never depends on fs.ts. All 490 unit tests + tsc clean. Backward-compat barrel re-exports the same symbol surface (createRun / getRun / listRuns / updateRunMeta / appendEvent / readEvents / findRunBySession from fs; deriveRunRow / deriveRunRowCached / deriveRunTokens / invalidateDerivedRow + DerivedRow + RunTokensBreakdown types from derive) so the ~33 callers across routes / server modules / tests need no edits. |
