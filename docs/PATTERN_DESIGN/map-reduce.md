...
| I1 | improvement | SHIPPED | (next commit) | — | `meta.enableSynthesisCritic` flag (default false) gates a peer-review loop after the synthesize item completes: picks any non-synthesizer session as critic, posts the synthesis + member drafts with a strict APPROVED/REVISE verdict format, parses the verdict (`parseCriticVerdict`); on REVISE re-prompts the synthesizer with the feedback (carries through `synthesisModel` pin if set), capped at 2 revisions. No new session spawn — reuses an existing peer to keep infra simple. Caps wait at 5 min per side; failures log and ship the current synthesis as final (fail-open). |
