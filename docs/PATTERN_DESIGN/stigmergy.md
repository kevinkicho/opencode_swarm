...
| heat-picked-timeline-chip | tab-eq | SHIPPED | (next commit) | — | placement deviation from spec — chip rendered on the **board-rail** row (where item state lives) instead of the timeline (which has complex event-derivation). Same user signal, simpler architecture. End-to-end: `BoardItem.pickedByHeat` field + sqlite `picked_by_heat` column + idempotent migration; coordinator detects heat-pick by comparing the picked item against age-only-first (cheap, deterministic) and sets the flag through `transitionStatus`; board-rail renders 🜂 amber glyph next to the row when set. Quiet on runs where heat agrees with age-only ordering anyway. |
