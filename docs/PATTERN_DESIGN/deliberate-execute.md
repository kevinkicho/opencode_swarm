...
| I3 | improvement | SHIPPED-BY-SHARE | (carried through council.md I3) | — | deliberation phase reuses `runCouncilRounds`, which builds R_max with the dissent-preservation instruction (`buildRoundPrompt isFinalRound=true`). Synthesis (phase 2) then reads dissent-preserving drafts as input — minority view propagates without a deliberate-execute-specific code change. |
