...
**On roles.** Roles are a design choice per run — available when a pattern needs them, absent when it doesn't. Both shapes are legitimate: self-organizing runs (blackboard, council, stigmergy) have no pinned roles — agents self-select what to work on; hierarchical runs (orchestrator-worker, role differentiation, debate+judge) have explicit roles and named hierarchy. The human picks the pattern that fits the work; the pattern picks the role model. An agent always has a **name** and an optional self-authored **focus line** ("reviewing the router diff"); in hierarchical patterns it may additionally carry a **role** (`orchestrator`, `worker`, `judge`, etc.) that scopes what it does. The human operator scans names, roles where present, status dots, focus lines, and timeline events.

*History note (2026-04-23):* this section previously rejected supervisor-worker and role pinning categorically — declared "the single most load-bearing stance in the project." The stance was reversed by the user after a long overnight run demonstrated that agents produce stronger work when a planner has explicit authority to organize the team. The earlier principle — human seeds direction + bounds, agents self-select — still governs self-organizing patterns; it just stopped being the *only* legitimate shape. See `memory/feedback_no_role_hierarchy.md` for the rationale in full.
...
- **Provider tier vocabulary** — three tiers, distinguished by billing shape, not model vendor: `zen` (pay-per-token marketplace), `go` (opencode subscription bundle), `ollama` (ollama.com max subscription — the 5 `:cloud` models: nemotron-3-super, gemma4:31b, kimi-k2.6, glm-5.1, mistral-large-3). All three route through opencode; the user configures `opencode.json` with an `ollama` provider block before ollama-tier models route cleanly. We do not expose BYOK as a selection affordance. History note: `zen + go only` was the stance through 2026-04-23; reversed 2026-04-24 when the ollama-max cost shape made it the strictly-better substrate for sustained runs — see §9.
...
- **`shape: 'diffs'` (shipped 2026-04-21)** filters to `part_type='patch'` and hydrates each returned item with `hunks: Array<{filePath, patch}>` — the session-aggregate unified diff captured at rollup time from `/session/{id}/diff`. Storage is per-`(session_id, file_path)` in the `diffs` table (composite PK, so re-rollup replaces rather than accumulates); capture is best-effort inside `generateRollup()` — fetch failures log and move on, the rollup's counters still land. Patch parts whose session hasn't been rolled up yet return with `hunks: []`. Each file's diff is truncated at 16 KB on write (bounds memory DB size + recall token cost); agents that need untruncated hunks hit `/session/{id}/diff` directly. Per-patch (not per-session) granularity isn't retrievable at all — opencode's endpoint ignores `?messageID=` / `?hash=` (probed 2026-04-20), so every patch-part in the same session surfaces the *same* hunk text for a given file.
...
- **Provider universe = zen + go + ollama.** Three tiers, all routed through opencode (user configures `opencode.json` for the ollama provider block; this app just surfaces it as a selectable tier). No BYOK or local-model UI at selection surfaces. History: zen + go only was the load-bearing stance through 2026-04-23; reversed 2026-04-24. Why: opencode-go usage ceilings cap a lot lower than sustained runs need, and opencode-zen pay-per-token is affordable but nowhere near ollama-max's monthly-flat economics. Adding ollama as a third tier preserves the "multi-configuration tool" shape the reversal opts into — see WHAT_THIS_PROJECT_IS_NOT.md for the scope clarification. Agents are still configured by name, never by source.
...
- **Roles are pattern-scoped, not universal.** Self-organizing runs (blackboard, council, stigmergy) have no pinned roles — agents self-select providers within run bounds. Hierarchical runs (orchestrator-worker, role differentiation, debate+judge, critic loops) CAN pin roles when the pattern needs them, and the roles ARE visible in the UI (role label on the roster, named hierarchy in the timeline). There is an `AgentRole` field on `Agent` for use by hierarchical patterns; it MUST be left empty on self-organizing runs. Routing remains bounds-driven (cost caps, time caps, workspace scope); `if role=X → provider=Y` is still off-limits because role determines *what an agent does*, not *which model it runs on*. History: this point previously said "no roles, ever" — reversed 2026-04-23; see §1 "On roles" for the full story.
