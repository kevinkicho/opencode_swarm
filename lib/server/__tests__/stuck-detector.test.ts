...
describe('detectStuckDeliberation', () => {
  ...
  it('catches the MAXTEAM-2026-04-26 deliberate-execute case', () => {
    // From docs/STRESS_TESTS/2026-04-26-max-team-size-8.md:
    // "deliberate-execute teamSize=8 → 3.4M tokens / 0 done, never
    // reached phase 2." Should fire stuck.
    const r = detectStuckDeliberation({
      tokensTotal: 3_400_000,
      ageMs: 30 * 60 * 1000,
      boardItemCount: 0,
  ...
  it('catches the MAXTEAM-2026-04-26 map-reduce case', () => {
    ...
    // aggressively than the deliberate-execute case.
    const r = detectStuckDeliberation({
