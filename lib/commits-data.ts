...
export type CommitSession = {
  id: string;
  agent: string;
  // Position in the delegation graph: `root` = session the human seeded;
  // `spawned` = session created by another agent via the `task` tool.
  // Structural — describes the graph, not an identity or permission.
  origin: 'root' | 'spawned';
...
export function rootAgent(c: CommitRecord): string {
  return (
    c.sessions.find((s) => s.origin === 'root')?.agent ??
    c.sessions[0]?.agent ??
    'unknown'
  );
}
...
export const commits: CommitRecord[] = [
  {
    ...
    duration: '12m 14s',
    sessions: [
      { id: 's1a', agent: 'ember', origin: 'root', tokens: split(52000), cost: 0.53, shipped: true },
      { id: 's1b', agent: 'forge', origin: 'spawned', tokens: split(46000), cost: 0.5, shipped: true },
      { id: 's1c', agent: 'atlas', origin: 'spawned', tokens: split(32000), cost: 0.32, shipped: true },
      { id: 's1d', agent: 'keel', origin: 'spawned', tokens: split(20000), cost: 0.22, shipped: true },
      { id: 's1e', agent: 'orbit', origin: 'spawned', tokens: split(12300), cost: 0.15, shipped: true },
      { id: 's1f', agent: 'orbit', origin: 'spawned', tokens: split(22000), cost: 0.15, shipped: false },
    ],
    files: [
    ...
    actions: [
      ...
      },
      {
        ...
        body: 'Likely culprit: retry on 5xx response causing the worker to double-insert. Dispatch orbit for log triage, atlas for handler review.',
      },
      {
        id: 'a3',
        timestamp: '14:32:40',
        kind: 'delegate',
        agent: 'ember',
        title: 'dispatch orbit pull last 24h of webhook logs',
      },
      {
        id: 'a4',
        timestamp: '14:33:12',
      ...
      },
      {
        ...
        kind: 'delegate',
        agent: 'ember',
        title: 'dispatch atlas audit webhook handler',
      },
      {
        id: 'a7',
        timestamp: '14:35:01',
      ...
      },
      {
        ...
        kind: 'delegate',
        agent: 'ember',
        title: 'dispatch forge implement ledger + guard',
      },
      {
        id: 'a10',
        timestamp: '14:37:30',
  ...
  },
  {
    ...
    duration: '1h 38m',
    sessions: [
      { id: 's2a', agent: 'ember', origin: 'root', tokens: split(130000), cost: 1.05, shipped: true },
      { id: 's2b', agent: 'forge', origin: 'spawned', tokens: split(210000), cost: 1.8, shipped: true },
      { id: 's2c', agent: 'atlas', origin: 'spawned', tokens: split(112000), cost: 0.92, shipped: true },
      { id: 's2d', agent: 'keel', origin: 'spawned', tokens: split(60400), cost: 0.45, shipped: true },
    ],
    files: [
    ...
    actions: [
      ...
      },
      {
        ...
        kind: 'delegate',
        agent: 'ember',
        title: 'dispatch forge write Clerk adapter',
      },
      {
        id: 'b6',
        timestamp: '11:18:44',
  ...
  },
  {
    ...
    summary:
      'Added Prom histogram for middleware latency, dashboard JSON committed, oncall runbook updated. orbit validated the p99 alert fires against synthetic load.',
    timestamp: '2026-04-17 16:45:02',
    relative: '2 days ago',
    status: 'success',
    duration: '42m',
    sessions: [
      { id: 's3a', agent: 'atlas', origin: 'root', tokens: split(52000), cost: 0.38, shipped: true },
      { id: 's3b', agent: 'forge', origin: 'spawned', tokens: split(68000), cost: 0.46, shipped: true },
      { id: 's3c', agent: 'orbit', origin: 'spawned', tokens: split(28200), cost: 0.2, shipped: true },
    ],
    files: [
  ...
  },
  {
    ...
    duration: '2h 14m',
    sessions: [
      { id: 's4a', agent: 'ember', origin: 'root', tokens: split(98000), cost: 0.8, shipped: true },
      { id: 's4b', agent: 'forge', origin: 'spawned', tokens: split(162000), cost: 1.24, shipped: true },
      { id: 's4c', agent: 'orbit', origin: 'spawned', tokens: split(62000), cost: 0.52, shipped: true },
      { id: 's4d', agent: 'keel', origin: 'spawned', tokens: split(56400), cost: 0.5, shipped: true },
    ],
    files: [
  ...
  },
  {
    ...
    duration: '38m',
    sessions: [
      { id: 's5a', agent: 'atlas', origin: 'root', tokens: split(40000), cost: 0.28, shipped: true },
      { id: 's5b', agent: 'forge', origin: 'spawned', tokens: split(16000), cost: 0.14, shipped: true },
      { id: 's5c', agent: 'forge', origin: 'spawned', tokens: split(40200), cost: 0.3, shipped: false },
    ],
    files: [
